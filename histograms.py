...
def calculate_lab_hist(listIm):
    histLab = []
    bins = 8
    for img in listIm:
        lab = cv2.cvtColor(img, cv2.COLOR_BGR2LAB)
        hist = cv2.calcHist([lab], [0, 1, 2], None, [2*bins, bins, int(bins/2)], [0, 256, 0, 256, 0, 256])
        hist = cv2.normalize(hist, hist)
        histLab.append(hist)
...
def hsv_componentByComponent(listIm):
    histHSV = []
    for img in listIm:
        hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
        hist = cv2.calcHist([hsv],[0,1,2],None,[128,64,64],[0,256,0,256,0,256])
        hist = cv2.normalize(hist, hist)
        #histHSV.append(hist.flatten())
        histHSV.append(hist)
    return histHSV
